...
{
public:
	BeladyCache(vector<string>& file, vector<int>& next_access_list, int cap): _file(file), _next_access_list(next_access_list), _capacity(cap), _time(0) {}
	~BeladyCache() {
	...
	string set(string key) {
		...
		if (_isCached) {
			...
			// item is not in cache, need to make eviction decision
